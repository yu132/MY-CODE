...
public class IterativeDeepeningSearch <T extends NormalSearchable<I>&Indexable<I,RT>, RT, I> implements Non_informationSearcher<T, RT, I>{
	...
	public I search(I startIndex, T container, SearchAim<I> aim) {
		...
		Deque<I> nextStartQueue = new ArrayDeque<>();
		nextStartQueue.addLast(startIndex);
		while(!nextStartQueue.isEmpty()){
			Deque<Counter> layerStack = new ArrayDeque<>();
			int layer=0;
			Counter nowCounter=new Counter(1);
			Set<I> hasSearchIndexSet=new TreeSet<>();
			stack.push(nextStartQueue.removeFirst());
			while(!stack.isEmpty()){
				I index=stack.pop();
	...
	public IterativeDeepeningSearch(int maxLayer) {
		super();
		this.maxLayer = maxLayer;
	}

	public IterativeDeepeningSearch(SearchDetector<I> searchDetector, int maxLayer) {
		super();
		this.searchDetector = searchDetector;
		this.maxLayer = maxLayer;
	}
